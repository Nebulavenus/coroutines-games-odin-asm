...
| **Strategy F: Effective Address Arithmetic (`lea`)** | `lea cur_sp, [%rsp + 0]` | **Successfully reads `%rsp`!** (Used in our stack calculations). | While `lea` solves reading `%rsp` as a memory base operand, it cannot be used to load or save general registers like `%rbp`, `%rbx`, `%rsi`, `%rdi`, `%r12`..`%r15`. |
