...
- **Native Stackful Coroutine Engine (`coroutine` package)**:
  ...
  - SkookumScript-inspired Structured Concurrency & Higher-Level Primitives (`api.odin`):
  ...
  - Exhaustive 33-test unit suite (`coroutine_test.odin`) covering 4-tier deep hierarchies (`sync`/`race`), 1000-timer min-heap stress, random mid-sleep cancellations, multi-slab expansion (15+ slabs), 10-generation pool reuse, stack isolation across 50 fibers, heterogeneous scope cancellation, temporary allocator isolation across yields, timeouts, signals, mutexes, and stack watermark telemetry.
